...
import { formatDob, toIsoDateInputValue } from "@/lib/formatDate";
...
export default function ClassesPage() {
  ...
  // Add/Sửa học sinh modal state — dùng chung 1 modal cho cả 2 luồng, phân
  // biệt bằng editingStudentId (null = đang thêm mới, có id = đang sửa).
  ...
  const [newStudentHobbies, setNewStudentHobbies] = useState("");
  ...
  const resetStudentForm = () => {
    ...
    setNewStudentName("");
    setNewStudentGender("Bé trai");
    setNewStudentDob("");
    setNewStudentAddress("");
    setNewStudentHobbies("");
  ...
  const openEditStudent = (student: any) => {
    ...
    setNewStudentGender(student.gender || "Bé trai");
    setNewStudentDob(toIsoDateInputValue(student.dateOfBirth));
    setNewStudentAddress(student.address || "");
    setNewStudentHobbies(student.hobbies || "");
    setNewStudentParentName(student.parentName || "");
    setNewStudentParentPhone(student.parentPhone || "");
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (!editingStudentId && !classes[0]) return;
    ...
    try {
      const res = editingStudentId
        ...
        : await fetch("/api/classes", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ classId: classes[0].id, ...payload }),
          });
      ...
      if (data.success) {
        ...
        alert(data.error || (editingStudentId ? "Không thể cập nhật học sinh" : "Không thể thêm học sinh"));
      }
    } catch (err) {
      console.error(err);
    } finally {
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        >
                          {student.gender || "Bé"} • {formatDob(student.dateOfBirth) || "Chưa cập nhật ngày sinh"}
                        </p>
                      </div>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-emerald-50">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Ngày sinh: {formatDob(selectedStudent.dateOfBirth) || "Chưa cập nhật"} • {selectedStudent.gender}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1.5 text-[11px] text-slate-500">
                        ...
                        {selectedStudent.hobbies && (
                          <span className="inline-flex items-center gap-1">
                            ...
                            Sở thích: {selectedStudent.hobbies}
                          </span>
                        )}
                        {selectedStudent.parentName && (
                          <span className="inline-flex items-center gap-1">
                            <Users className="w-3 h-3 text-slate-400" />
                            PH: {selectedStudent.parentName}
                          </span>
                        )}
                      ...
                      {selectedStudent.observations.map((obs: any) => (
                        ...
                        >
                          ...
                          <p className="text-[10px] text-slate-400 text-right">
                            Ghi bởi: {obs.createdBy || "Giáo viên"}
                          </p>
                        </div>
                      ))}
                    </div>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAddStudent} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ví dụ: 12 Nguyễn Trãi, Q.1, TP.HCM"
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Sở thích của bé</label>
                <input
                  type="text"
                  placeholder="Ví dụ: Vẽ tranh, xếp hình, nghe nhạc..."
                  value={newStudentHobbies}
                  onChange={(e) => setNewStudentHobbies(e.target.value)}
                  className="w-full px-3 py-2 rounded-xl border border-slate-200"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Tên phụ huynh</label>
                  <input
                    type="text"
                    placeholder="Ví dụ: Chị Nguyễn Thị Hoa"
