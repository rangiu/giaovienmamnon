...
function ToolsContent() {
  ...
  const [commentStudentName, setCommentStudentName] = useState(initialName || "bé Nguyễn Minh");
  const [commentRawInput, setCommentRawInput] = useState(
    "Hôm nay bé ăn tốt, ngủ bình thường nhưng hơi nhút nhát khi chơi nhóm."
  );
  ...
  const [parentStudentName, setParentStudentName] = useState(initialName || "bé Nguyễn Minh");
  const [parentRawInput, setParentRawInput] = useState(
    "Bé hôm nay ăn ít và hơi mệt."
  );
  ...
  // Media Prompt State
  const [mediaRequirement, setMediaRequirement] = useState("Quả cam tươi mọng nước trên đĩa nhựa học tập");
  const [mediaType, setMediaType] = useState<"image" | "video">("image");
  const [artStyle, setArtStyle] = useState("3d_cute");
          ...
          <div className="bg-white p-6 rounded-3xl border border-indigo-100 shadow-sm space-y-4">
            ...
            <form onSubmit={handleGenerateMediaPrompt} className="space-y-4 text-xs">
              ...
              <div>
                ...
                >
                  <option value="3d_cute">🧸 Hoạt hình 3D ngộ nghĩnh, màu sắc tươi sáng</option>
                  <option value="watercolor">🎨 Tranh vẽ màu nước mộng mơ mầm non</option>
                  <option value="coloring_book">✏️ Tranh nét tô màu đen trắng cho trẻ</option>
                  <option value="3d_clay">🎨 Đất nặn 3D sinh động dễ thương</option>
                  <option value="flat_vector">⭐ Tranh 2D phẳng tươi sáng (Dạng Flashcard)</option>
                </select>
              </div>
...
export default function ToolsPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-sm text-slate-500">Đang tải công cụ Cô AI...</div>}>
      <ToolsContent />
    </Suspense>
  );
}
